  ...
  if (link.childrens) {
    return (
      ...
      >
        ...
        >
          <FaChevronDown
            className={`${openAboutUs ? "hidden" : ""} absolute left-8 cursor-pointer transition-transform duration-500 ${open ? "rotate-180" : ""}`}
            onClick={
              link.title == "عن الجمعية"
          ...
          <FaArrowRight
            className={`${openAboutUs ? `${() => setOpen(true)}` : "hidden"} absolute right-0 text-lg cursor-pointer transition-transform duration-500 `}
            onClick={() =>
        ...
        >
          {link.childrens.map((child, index) => (
            <SideBarItems key={index} link={child} setOpenAboutUs={setOpen} />
          ))}
        </div>
      </div>
    );
