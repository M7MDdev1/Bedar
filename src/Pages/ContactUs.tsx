      ...
      >
        {({ isSubmitting }) => (
          <Form  className="w-full bg-[#C58F3526] flex flex-col text-right p-4 relative">
            {Fields.map((field) => (
              <>
                <label htmlFor={field.id} className="m-3">{field.name}</label>
                <Field type={field.type} id={field.id} name={field.id} className="h-12 rounded-2xl p-4 text-right border border-[#c58f35ad]"/>
                <ErrorMessage name={field.id} component="div"/>
              </>
            ))}
            <>
              <label htmlFor='content' className="m-3">محتوى الرسالة</label>
              <Field as="textarea" id='content' name='content' className="h-[192px] rounded-2xl p-4 text-right border border-[#c58f35ad]"/>
            </>
            <button type='submit' disabled={isSubmitting} className='w-48 h-14 my-12 mx-auto bg-[#C58F35] text-white rounded-[20px] shadow-[2px_2px_5px_0px_rgba(197,143,53,0.68)]'>
