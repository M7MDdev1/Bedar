                                ...
                                <>
                                    {field.width === "half" ? (
                                        <div className="flex gap-2 flex-row-reverse">
                                            <div>
                                                <Field type={field.type} name={field.id[0]} id={field.id[0]} placeholder={field.name[0]} className="h-[46px] w-full text-sm text-right rounded-2xl border border-amber-500/70 p-3 cursor-pointer"></Field>
                                                <ErrorMessage name={field.id[0]} component="div" className="text-red-400 text-sm"/>
                                            </div>

                                            <div>
                                                <Field type={field.type} name={field.id[1]} id={field.id[1]} placeholder={field.name[1]} className="h-[46px] w-full text-sm text-right rounded-2xl border border-amber-500/70 p-3 cursor-pointer"></Field>
                                                <ErrorMessage name={field.id[1]} component="div" className="text-red-400 text-sm"/>
                                            </div>
                                        </div>
                                    ) : (
                                        <>
                                        <Field type={field.type} name={field.id} id={field.id} placeholder={field.name} className="h-[46px] text-right text-sm rounded-2xl border border-amber-500/70 p-3 cursor-pointer"></Field>
                                        <ErrorMessage name={field.id +""} component="div" className="text-red-400 text-sm"/>
                                    </>
                                    )}
                                </>
                            ))}
                            <Field as="textarea" name="aboutTeam" id="aboutTeam" placeholder="..نبذة عن فريقك التطوعي" className="h-[115px] w-full text-sm text-right rounded-2xl border border-amber-500/70 p-3 cursor-pointer"></Field>
                            <ErrorMessage name="aboutTeam" component="div" className="text-red-400 text-sm"/>

                            {/* Form Footer */}
                            <div className="relative text-right mb-8 px-4">
                                <h3 className="text-center text-[#C58F35] font-bold my-8"><span className="block">خذ فريقك</span> التطوعي إلى عوالم لانهائية</h3>
                                <div className="flex items-center justify-end mb-[14px]">
                                    <ErrorMessage name="eqrar" component="div" className="text-red-400 text-sm"/>
                                    <label htmlFor="eqrar" className="text-black font-bold mx-3">إقرار</label>
                                    <Field type="checkbox" name="eqrar" id="eqrar" className="hidden peer"/>
